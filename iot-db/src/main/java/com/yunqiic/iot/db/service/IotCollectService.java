...
import com.yunqiic.iot.db.dao.IotCollectMapper;
...
@Service
public class IotCollectService {
    @Resource
    private IotCollectMapper collectMapper;

    public int count(int uid, Integer gid) {
        IotCollectExample example = new IotCollectExample();
        example.or().andUserIdEqualTo(uid).andValueIdEqualTo(gid).andDeletedEqualTo(false);
        return (int) collectMapper.countByExample(example);
    }

    public List<IotCollect> queryByType(Integer userId, Byte type, Integer page, Integer limit, String sort, String order) {
        IotCollectExample example = new IotCollectExample();
        IotCollectExample.Criteria criteria = example.createCriteria();
    ...
    public IotCollect queryByTypeAndValue(Integer userId, Byte type, Integer valueId) {
        IotCollectExample example = new IotCollectExample();
    ...
    public int add(IotCollect collect) {
        collect.setAddTime(LocalDateTime.now());
        collect.setUpdateTime(LocalDateTime.now());
        return collectMapper.insertSelective(collect);
    }

    public List<IotCollect> querySelective(String userId, String valueId, Integer page, Integer size, String sort, String order) {
        IotCollectExample example = new IotCollectExample();
        IotCollectExample.Criteria criteria = example.createCriteria();
