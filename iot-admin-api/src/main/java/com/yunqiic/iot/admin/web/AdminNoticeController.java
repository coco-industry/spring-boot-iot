...
import com.yunqiic.iot.db.service.IotNoticeAdminService;
import com.yunqiic.iot.db.service.IotNoticeService;
...
import static com.yunqiic.iot.admin.util.AdminResponseCode.NOTICE_UPDATE_NOT_ALLOWED;
...
public class AdminNoticeController {
    ...
    @Autowired
    private IotNoticeService noticeService;
    @Autowired
    private IotAdminService adminService;
    @Autowired
    private IotNoticeAdminService noticeAdminService;
    ...
    public Object list(String title, String content,
        ...
        List<IotNotice> noticeList = noticeService.querySelective(title, content, page, limit, sort, order);
        return ResponseUtil.okList(noticeList);
    }

    private Object validate(IotNotice notice) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody IotNotice notice) {
        ...
        // 2. 添加管理员通知记录
        List<IotAdmin> adminList = adminService.all();
    ...
    public Object read(@NotNull Integer id) {
        IotNotice notice = noticeService.findById(id);
        List<IotNoticeAdmin> noticeAdminList = noticeAdminService.queryByNoticeId(id);
    ...
    @PostMapping("/update")
    public Object update(@RequestBody IotNotice notice) {
        Object error = validate(notice);
        if (error != null) {
            return error;
        }
        IotNotice originalNotice = noticeService.findById(notice.getId());
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody IotNotice notice) {
        // 1. 删除通知管理员记录
