...
import com.yunqiic.iot.wx.vo.CouponVo;
import com.yunqiic.iot.wx.util.WxResponseCode;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<IotCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<IotCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<IotCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for(IotCouponUser couponUser : couponList){
            Integer couponId = couponUser.getCouponId();
            IotCoupon coupon = couponService.findById(couponId);
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        for (CouponVo cv : couponVoList) {
            IotCoupon coupon = couponVerifyService.checkCoupon(userId, cv.getCid(), cv.getId(), checkedGoodsPrice);
    ...
    public Object receive(@LoginUser Integer userId, @RequestBody String body) {
        ...
        // 用户领券记录
        IotCouponUser couponUser = new IotCouponUser();
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        IotCoupon coupon = couponService.findByCode(code);
        if(coupon == null){
