...
public class WxHomeController {
    ...
    @Autowired
    private IotAdService adService;
    ...
    private List<Map> getCategoryList() {
        ...
        List<IotCategory> catL1List = categoryService.queryL1WithoutRecommend(0, SystemConfig.getCatlogListLimit());
        for (IotCategory catL1 : catL1List) {
            List<IotCategory> catL2List = categoryService.queryByPid(catL1.getId());
            List<Integer> l2List = new ArrayList<>();
            for (IotCategory catL2 : catL2List) {
                l2List.add(catL2.getId());
            }

            List<IotGoods> categoryGoods;
            if (l2List.size() == 0) {
