...
import com.yunqiic.iot.core.express.ExpressService;
import com.yunqiic.iot.core.express.dao.ExpressInfo;
import com.yunqiic.iot.core.notify.NotifyService;
import com.yunqiic.iot.core.notify.NotifyType;
...
import com.yunqiic.iot.core.util.DateTimeUtil;
...
import com.yunqiic.iot.db.util.OrderHandleOption;
import com.yunqiic.iot.db.util.OrderUtil;
import com.yunqiic.iot.core.util.IpUtil;
import com.yunqiic.iot.wx.task.OrderUnpaidTask;
...
import static com.yunqiic.iot.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    @Autowired
    private IotOrderGoodsService orderGoodsService;
    @Autowired
    private IotAddressService addressService;
    @Autowired
    private IotCartService cartService;
    @Autowired
    ...
    @Autowired
    private IotAftersaleService aftersaleService;
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<IotOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (IotOrder o : orderList) {
            ...
            IotGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<IotOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        IotOrder order = orderService.findById(userId, orderId);
        if (null == order) {
        ...
        List<IotOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            IotGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                IotGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        // 收货地址
        IotAddress checkedAddress = addressService.query(userId, addressId);
        ...
        IotGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<IotCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            IotCart cart = cartService.findById(cartId);
        ...
        for (IotCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            IotCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        // 添加订单商品表项
        for (IotCart cartGoods : checkedGoodsList) {
            // 订单商品
            IotOrderGoods orderGoods = new IotOrderGoods();
        ...
        for (IotCart checkGoods : checkedGoodsList) {
            ...
            IotGoodsProduct product = productService.findById(productId);
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            IotCouponUser couponUser = couponUserService.findById(userCouponId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            IotGroupon groupon = new IotGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                IotGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        IotOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        IotGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            IotGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
            ...
            List<IotGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
            if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                for (IotGroupon grouponActivity : grouponList) {
                    grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                    grouponService.updateById(grouponActivity);
                }

                IotGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<IotOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        IotOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        IotOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        IotComment comment = new IotComment();
